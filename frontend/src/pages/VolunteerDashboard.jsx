    ...
    <div className="min-h-screen flex bg-slate-50 dark:bg-slate-950 animate-fade-in transition-colors">
      ...
      <div className="flex-grow overflow-auto">
        <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-8 py-4 flex items-center justify-between">
          ...
          <button onClick={fetchAll} className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white" title="Refresh">
        ...
        <div className="lg:hidden flex gap-2 px-6 pt-4">
          {['available', 'mine'].map(k => (
            <button
              key={k}
              onClick={() => setTab(k)}
              className={`px-4 py-2 rounded-lg text-xs font-bold capitalize ${tab === k ? 'bg-blue-600 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}
            >
